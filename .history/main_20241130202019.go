...
func selectArticleHandler(w http.ResponseWriter, r *http.Request) {
	...
	db := openDb()
	defer db.Close()
	t, a, b := getArticle(db, title)

	htmlStr := fmt.Sprintf("<div class='inline-block w-full rounded bg-pink-700 px-6 pb-2.5 pt-2.5 text-lg font-large uppercase leading-normal text-white m-2'> %s - %s <p>%s</p></div>", t, a, b)
	tpl, _ := template.New("t").Parse(htmlStr)
	tpl.Execute(w, nil)
}
