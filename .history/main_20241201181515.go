...
func articlelistHandler(w http.ResponseWriter, r *http.Request) {
	...
	tpl, _ := template.New("t").Parse(`
	{{range .}}
	<form hx-post="/article/" hx-target="#article-box" hx-swap="beforeend" hx-on::after-request="this.reset()"><input type="text" placeholder={{.Title}} class="peer block min-h-[auto] w-full rounded border-0 bg-pink-700 px-3 py-[0.32rem] leading-[1.6] outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 peer-focus:text-primary data-[twe-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none dark:text-black dark:placeholder:text-black dark:autofill:shadow-autofill dark:peer-focus:text-primary [&:not([data-twe-input-placeholder-active])]:placeholder:opacity-1">
	<button type="submit"inline-block w-full rounded bg-pink-700 px-6 pb-2 m-1 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-primary-3 transition duration-150 ease-in-out hover:bg-primary-accent-300 hover:shadow-primary-2 focus:bg-primary-accent-300 focus:shadow-primary-2 focus:outline-none focus:ring-0 active:bg-primary-600 active:shadow-primary-2 dark:shadow-black/30 dark:hover:bg-pink-500 shadow-dark-strong dark:focus:shadow-dark-strong dark:active:shadow-dark-strong>Select</button>
	</form>
	{{end}}`)
