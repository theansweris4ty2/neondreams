...
func articlelistHandler(w http.ResponseWriter, r *http.Request) {
	...
	tpl, _ := template.New("t").Parse(`
		{{range .}}
	<li class='inline-block w-full rounded bg-pink-700 px-6 pb-2.5 pt-2.5 text-lg font-large uppercase leading-normal text-white m-px max-w-screen-sm'>{{.Title}} - {{.Author}}<form hx-post="/article/" "hx-target="article-box" hx-swap="innerHTML" value="{{.Title}}><input type="hidden" name="title> value="{{.Title}}"> <button type="submit"
