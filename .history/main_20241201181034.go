...
func articlelistHandler(w http.ResponseWriter, r *http.Request) {
	...
	tpl, _ := template.New("t").Parse(`
	{{range .}}
	<form hx-post="/article/" hx-target="#article-box" hx-swap="beforeend" hx-on::after-request="this.reset()"><input type="text" placeholder={{.Title}} class="peer block min-h-[auto] w-full rounded border-0 bg-pink-700 px-3 py-[0.32rem] leading-[1.6] outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 peer-focus:text-primary data-[twe-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none dark:text-black dark:placeholder:text-black dark:autofill:shadow-autofill dark:peer-focus:text-primary [&:not([data-twe-input-placeholder-active])]:placeholder:opacity-1"></form>
	{{end}}`)
