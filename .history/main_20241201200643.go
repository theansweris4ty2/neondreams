...
func articlelistHandler(w http.ResponseWriter, r *http.Request) {
	...
	tpl, _ := template.New("t").Parse(`
		{{range .}}
	<li class='inline-block w-full rounded bg-pink-700 px-6 pb-2.5 pt-2.5 text-lg font-large uppercase leading-normal text-white m-px max-w-screen-sm'>{{.Title}} - {{.Author}}<button  hx-post="/article/" hx-trigger="click" hx-target="article-box" hx-swap="innerHTML" value="{{.Title}}"class="inline-block w-full rounded bg-gray-700 m-1 px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-primary-3 transition duration-150 ease-in-out hover:bg-primary-accent-300 hover:shadow-primary-2 focus:bg-primary-accent-300 focus:shadow-primary-2 focus:outline-none focus:ring-0 active:bg-primary-600 active:shadow-primary-2 dark:shadow-black/30 dark:hover:bg-gray-500 shadow-dark-strong dark:focus:shadow-dark-strong dark:active:shadow-dark-strong">Select</button></li>
		{{end}}`)
