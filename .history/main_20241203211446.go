...
func selectShow(w http.ResponseWriter, r *http.Request) {
	...
	defer db.Close()
	t := getShow(db, title)
	htmlStr := fmt.Sprintf("<li class='inline-block w-full rounded bg-pink-700 px-6 pb-2.5 pt-2.5 text-lg font-large uppercase leading-normal text-white m-2'> %s</li>", t)
	tpl, _ := template.New("t").Parse(htmlStr)
	tpl.Execute(w, nil)
}
...
func showlistHandler(w http.ResponseWriter, r *http.Request) {
	...
	tpl, _ := template.New("t").Parse(`
		{{range .}}	
	<li class='inline-block w-full rounded bg-pink-700 px-6 pb-2.5 pt-2.5 text-lg font-large uppercase leading-normal text-white m-2'>{{.Title}}</li>
		{{end}}`)
	tpl.Execute(w, shows)
