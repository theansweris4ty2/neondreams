...
func selectArticleHandler(w http.ResponseWriter, r *http.Request) {
	...
	htmlStr := fmt.Sprintf(`<div class='inline-block w-full rounded bg-pink-700 px-6 pb-2.5 pt-2.5 text-lg font-large uppercase leading-normal text-white m-2'> <p>%s</p><button type="submit"
	class="inline-block w-full rounded bg-gray-700 m-1 px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-primary-3 transition duration-150 ease-in-out hover:bg-primary-accent-300 hover:shadow-primary-2 focus:bg-primary-accent-300 focus:shadow-primary-2 focus:outline-none focus:ring-0 active:bg-primary-600 active:shadow-primary-2 dark:shadow-black/30 dark:hover:bg-gray-500 shadow-dark-strong dark:focus:shadow-dark-strong dark:active:shadow-dark-strong" hx-post="/articles-list/" hx-target="article-list" hx-swap="innerHTML">Return to Start</button>
	</div>`, b)
	tpl, _ := template.New("t").Parse(htmlStr)
	tpl.Execute(w, nil)
}
